<template>
    ...
    :form_modal="form_modal" :cancelMethod="cancel">
        <div class="form-group">
            ...
            id="section_name_ipt" class="form-control" required>
...
export default {
    ...
    methods: {
        getSection(){
            SectionService.get(this.id).then(
                (response) => {
        ...
        validations(){
            ...
            if(this.desc.trim().length < 10){
                this.inputErrors.push("Longitud de la descripción debe ser minimo de 10");
            }
            if(this.video.trim().length < 0){
                this.inputErrors.push("Longitud de la url  del video debe ser mayor a 0");
            }
        ...
        save(){
            ...
            SectionService.update(this.id,this.name,this.desc,this.video).then(
                (response) => {
                    Swal.fire({
