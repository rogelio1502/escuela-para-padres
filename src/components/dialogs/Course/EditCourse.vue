<template>
    ...
    :wait_for_validation="cancel_save" :info="info" 
    ...
    :prop_disabled="disabled">
        <div class="form-group">
            ...
            <textarea v-model="course_desc" 
            name="course_desc" id="course_desc_ipt" 
            ...
            <br>
            <button class="btn btn-info" @click="$refs.fileInput.click()" type="button">
                Imagen de Portada
            </button>
            {{fileImg}}
...
export default {
    ...
    methods: {
        getCourse(){
            CourseService.getCourse(this.course_id).then(
                (response)=> {
        ...
        validations(){
            ...
            if(this.course_name.trim().length < 1){
                this.inputErrors.push("Longitud de Nombre del curso debe ser mayor a 0");
            }
            if(this.course_desc.trim().length < 10){
                this.inputErrors.push("Longitud de Descripción del curso debe ser mayor a 10")
            }
        ...
        save(){
            ...
            if(this.file !== null){
                ...
                reader.onloadend = (e)=>{
                    ...
                    console.log(result);
                    let json_data = {'name':this.course_name, "description":this.course_desc,"cover":result}
                    api.patch("course/" + this.course_id, json_data).then(
                        (response) => {
                            this.$emit('making_action',false)
                ...
                let json_data = {'name':this.course_name, "description":this.course_desc}
                api.patch("course/" + this.course_id, json_data).then(
