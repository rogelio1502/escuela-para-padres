...
class UserService {
  ...
  setProgress(email, course_id) {
    return api.patch("/user/course/set-progress/" + course_id + "/" + email, {
      user_email: email,
    });
  }

  setPersonalData(email, names, last_name, second_last_name) {
    //byself action
    ...
    return api.patch("/user/set-personal-data", json_data);
  }
  ...
  getPersonalData(email) {
    ...
    return api.post("/user/get-personal-data", json_data);
  }
